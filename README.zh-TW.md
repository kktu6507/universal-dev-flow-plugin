...
> **狀態:early / experimental(早期/實驗性)。** hook 有測試;多代理編排是 prompt 驅動。在外部 repo 上的第一次盲測結果不錯——**8 個真實 bug 抓到 6 個、0 誤報**(見下方 Evidence)——但樣本還小。請當成有紀律的鷹架。

<details>
<summary><b>Evidence(實測筆記)</b> — 第一次盲測,方向性結果(還不是保證)</summary>

**回溯盲測抓蟲(2026-06-19)。** 把 udflow 的審查員**盲測**在一個外部 C#/.NET repo(與本 Node plugin 不同棧)**8 個真實歷史 bug** 的「修復前」程式上。每位審查員只看到有問題的程式片段——看不到修復、issue、或整個 repo——再由一個中立 judge 拿「真相」評分。

| 8 個真實 bug,盲測 | 抓到 | 部分 | 漏掉 | 誤報 |
|---|---|---|---|---|
| 結果 | **6** | 1 | 1 | **0**(約 46 個 finding 中) |

- **抓到——具體、程式碼裡看得到的缺陷:** 未 dispose 的 `HttpResponseMessage`;未截斷就寫進 `nvarchar(4000)` 欄位的文字;灌進搜尋索引的原始 JSON;違反逐角色 spec 的寫死迭代上限;背景索引時永不刷新的 UI 計數;以及 model 沒有驗證屬性、導致 `DataAnnotationsValidator` 形同虛設的表單。
- **漏掉/部分——遺漏型/意圖型缺陷:** 漏清一個子表的串聯刪除,以及本該排除 binary 檔的統計。程式碼「看起來沒錯」,要知道意圖才抓得到——而且**完整的三位審查員 panel 也沒幫上忙**,這代表槓桿在於把**意圖/spec 餵給審查員**,而非加更多審查員。
- **0 誤報**,跨兩輪(約 46 個 finding;另有約 40 個「看似合理但未驗證」的 finding——是廣度訊號,不是已確認的抓到)。

**侷限:** n=8、單一 repo/單一語言;bug 取自 `fix` commits(部分本來就是過去 review 抓出來的),偏向「可被抓到」;未測並行/整合型 bug;每個 bug 只給單一審查員或小型 panel、**且無 plan/需求脈絡**,這會低估真實 udflow 流程。**方向性,非保證。**

**畢業條件**——當 udflow 在 **≥3 個外部 repo、≥2 種語言、≥20 個真實任務**上用過,並記錄抓蟲率與誤報率(*含並非由 review 先抓出的 bug*),就拿掉「experimental」標籤。
