...
- **深度模式** —— **Tier 1**（高風險 + 具 Workflow 能力時自動）：把*同一組*面板 + gatekeeper 以確定性 graph 跑,讓它真的依序執行（成本 ≈ 一般；`--no-deep` 退出）。**Tier 2**（`--deep`）：對 blocker/major 加對抗驗證 + 最高 effort + UI 需真實瀏覽器佐證。是加深度,不是加 reviewer。
...
| `ui-ux-reviewer` | 易用性、互動、版面、狀態、無障礙 | UI 影響 | inherit |
...
四個 Node hook —— **全部 fail-open**（任何錯誤、或 PATH 上沒 Node → 什麼都不做,絕不弄壞 session）、**純本地**（不連網、不開子行程、不跑下載程式碼；只用內建 `fs`/`os`/`path`/`crypto`）。只要 plugin 啟用,它們在*每一個* session 跑,非僅 udflow 任務。

| Hook（事件） | 它能做什麼 | 預設 · opt-out |
|---|---|---|
| `plan-gate.js`（PreToolUse） | **擋下**編輯 + *明顯的* Bash 寫檔,**只在 plan mode**；放行 `~/.claude/plans/` | 開 · `"udflow":{"planGate":false}` |
| `destructive-guard.js`（PreToolUse） | 在**任何**模式對不可復原 Bash **詢問**（永不硬擋）：`rm -rf`、`git reset --hard`、`git push --force`、`find -delete`、`dd of=`、`mkfs`、`shred` —— 以及 PowerShell 形式 `Remove-Item -Recurse` / `Format-Volume` / `Clear-Disk`（Windows / Copilot） | 開 · `"udflow":{"destructiveGuard":false}` |
...
| 旗標 | 效果 | 預設 |
|---|---|---|
| `--deep`（也可 `deep:` / `ultra:`） | Tier-2：對 blocker/major 對抗驗證 + 最高 effort；有 UI 時驅動真實瀏覽器佐證 | 關（高風險時 Tier-1 仍自動） |
...
udflow 以 **Claude Code** 為目標;其 **subagents** 與 **skills** 也能在 **GitHub Copilot CLI** 載入（1.0.65 實機驗證,2026-06-26 —— `plugin list`、`skill list`、列舉全部 9 個 subagent、兩個 hook 觀察到觸發）。跨 harness 載入依各工具有文件記載的 plugin 格式推得。
