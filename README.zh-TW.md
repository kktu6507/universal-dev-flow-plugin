...
| `precompact-fidelity.js`（PreCompact） | 在 context 壓縮前**注入**一段精簡指示,讓壓縮保留 udflow 自己的構件（reviewer 裁決、acceptance-criteria 狀態、`[unverified]` 標記、Run Card 數字、subagent 發現、未回答的需求）—— 只注入指示、不讀檔 | 開 · `"udflow":{"preserveOnCompact":false}` |
...
| 壓縮保真（`PreCompact`）hook | no-op —— 與 digest 同類：hook 會**載入**（0.27.1 實機驗證過）但注入輸出不送達,且 Copilot 未必觸發 `PreCompact` 事件;它 fail-open、絕不報錯。`output/udflow/progress.md` ledger 是連續性後備 |
