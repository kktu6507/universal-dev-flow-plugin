...
| Agent | 角色 | 何時加入 | 模型 |
|---|---|---|---|
| `planner-creator` | 用真實程式碼紮根計畫、草擬方法、預選面板、偵測 `design.md`（唯讀；輔助計畫核准，絕不取代） | 規劃 | inherit |
...
udflow 以 **Claude Code** 為目標;其 **subagents** 與 **skills** 也能在 **GitHub Copilot CLI** 載入（1.0.65 實機驗證,2026-06-26 —— `plugin list`、`skill list`、列舉全部 subagent、兩個 hook 觀察到觸發）。跨 harness 載入依各工具有文件記載的 plugin 格式推得。
