...
reviewer 對真實歷史 bug 的**修復前**程式碼**盲跑**（6 種語言／多個外部 repo）,由獨立評審對照已知修復計分。主要漏判類別（77-bug 語料）：omission 36% · 抓到*另一個*真實 bug 18% · 語言慣用 16% · 評級過低 15% · 領域知識 15%。限制：bug 多取自 `fix` commit；並行／整合幾乎沒測；許多次只用單一 reviewer、無計劃脈絡——這些都*低估*了完整一次 run。完整方法與逐次紀錄見 [`EVIDENCE.md`](EVIDENCE.md)（手動紀錄——udflow **無遙測**）。誠實標籤：一個*已被刻畫的*早期／beta——是方向性指標,不是保證。
...
兩個很不同的數字 —— **典型真實 app 工作**的數量級粗估,非保證。（對照 [`EVIDENCE.md`](EVIDENCE.md) 你會看到*更低*的實測值:那些 run 跑在 udflow 自己的小型 Markdown/Node repo、改動範圍小 —— ~0.1–1.5M new tokens,是本表的下限。codebase 更大、邏輯更複雜、修復迴圈更多時請往上估;下方 ~47M 是整個 P0–P3 build 跨 ~7 次 run,不是一次。）
