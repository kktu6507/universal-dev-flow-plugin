...
> **狀態:early / experimental(早期/實驗性)。** hook 有測試;多代理編排是 prompt 驅動。跨語言盲測(**6 種語言、32 個真實 bug**)顯示:**近乎零誤報**,但**單次抓得率不高(約 34%)**——udflow 的優勢是「精準(幾乎不誤報)+ 結構深度(多視角 panel / Deep Mode / 餵意圖)」,不是單一審查員的 recall(見下方 Evidence)。請當成有紀律的鷹架。
...
- **單次抓得率不高(約 34% hit、約 50% 碰到)且有上限。** 對「程式碼裡看得到」的缺陷(資源洩漏、DB 欄位溢位、形同虛設的驗證器)抓得穩;但漏掉**語言慣用陷阱**(錯誤的 `this`/receiver、char-vs-byte、lifetime、overflow)與**遺漏型**(相對意圖少做了什麼)。
- **拉高 recall 的是「結構」,不是審查員的措辭。** 在契約加上「就事論事、別合理化」的紀律,實測**安全(0 新增誤報)**但**沒**提高單人 recall;**多視角 panel** 卻能救回單人漏掉的缺陷(Deep Mode 與餵**意圖/spec** 再加分)。所以要抓得準就跑 panel,別只靠一個審查員。
