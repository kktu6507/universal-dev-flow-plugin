...
> **Status: early / experimental.** The hooks are tested; the multi-agent orchestration is prompt-driven. A cross-language benchmark (**6 languages, 32 real bugs**) shows **near-zero false positives**, with recall that hinges on one input: **diff-only ~34%, but ~84% (27/32) once the reviewer is given the intent/contract** — and real udflow supplies that intent via its Review Packet. False positives stayed at **0** in both conditions. See the Evidence note. Treat it as a disciplined scaffold.
...
- **The biggest recall lever is feeding the reviewer the intent — and real udflow already does.** Re-running the **same 32 bugs** with only an intent/contract note added (the required behavior, never the defect; nothing else changed) took recall from **11→27 hit (34% → 84%)** with **false positives still 0** — even subtle language-idiom cases flipped. So ~34% largely reflects *withheld intent*, not a capability ceiling; udflow's **Review Packet** delivers that intent to every reviewer. (A multi-lens **panel** and Deep Mode recover more still; a prose "don't rationalize" discipline *alone* was safe but did **not** move recall.) The 3 residual misses are genuinely hard (a type-system flag-conflation, a numeric-overflow case, and one where the reviewer found a *different* real bug). _Caveat: the intent notes were author-written; a stricter test would use each repo's own docs._
