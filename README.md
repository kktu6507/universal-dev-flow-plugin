...
|------|---------|--------|---------|
| `--deep` (also `deep:` / `ultra:`) | Tier-2 deep mode: adversarial verification of blocker/major findings + maximum reasoning effort; in `--deep` + UI it also drives **live browser evidence** via the Claude in Chrome extension (see [Deep mode](#deep-mode)) | ↑ recall on the hardest changes / ↑↑ cost + time | Off (Tier-1 deterministic enforcement still auto-engages on high-risk work when a Workflow capability is present) |
...
| `--report full` | Emit the detailed final report (per-agent activity, Files Changed, full cost table + share bar, Assumptions / Missing-Tests / Risks tables); the cost table breaks tokens into per-component **Input / Output / Cache-write / Cache-read** and embeds the changed-UI after-screenshots | Output: more output tokens; analysis is unchanged | Off (compact report by default) |
...
| `ui-ux-pro-max` skill | Design intelligence (styles, palettes, font pairings, UX) for UI work; **required consult for design-system / design-generation scope** (else disclosed fallback) | Quality: better UI design when present; built-in baseline otherwise | Used if installed |
| Claude in Chrome extension (`mcp__Claude_in_Chrome__*`; alternatives `mcp__Claude_Preview__*` / `mcp__playwright__*`) | Live browser evidence for UI verification — required in `--deep` + UI (Detect → Use → Else-Disclose) | Quality: real navigation + changed-state screenshots when present; disclosed gap otherwise. **Drives your real authenticated browser — reads/screenshots may expose secrets/PII; prefer a non-prod target, don't paste sensitive `--report full` screenshots into public PRs** | Used if connected |
| `output/udflow/evidence/` (consuming project) | Where `--deep` / `--report full` keep changed-UI screenshots referenced by the report | Behavior: kept evidence artifacts; recommend you `.gitignore` it | None (created on demand) |
...
- **Tier 2 — deeper verification (opt-in).** Prefix a task with `--deep` (e.g. `/udflow:run --deep <task>`) to *additionally* give blocker/major findings adversarial verification and run the highest-leverage agents at maximum effort. When UI is in scope it also makes **live browser evidence** a required verification step — the main thread drives a real browser (Claude in Chrome) and captures a screenshot of each changed UI state; an unavailable browser is a disclosed gap, never an error. This raises tokens/time, so it stays explicit opt-in.
...
`--report full` itemizes the cost table by billable component — **Input / Output / Cache-write / Cache-read** — since they price differently, so you can see where the spend actually lands.
...
- **ui-ux-pro-max**: if the `ui-ux-pro-max` skill is installed, udflow uses it first for UI design decisions; otherwise it falls back to a built-in baseline and discloses that. For **design-system / design-generation scope** (new screens or net-new component families, visual redesign, design tokens, theming, brand — a token-reusing one-off stays a small tweak) the consult is **required** at planning — its style / palette / font / UX decisions are recorded in the plan, or the fallback is disclosed (small UI tweaks keep the soft consult).
- **Claude in Chrome** (live browser evidence): when present, udflow drives a real browser for UI verification (Detect → Use → Else-Disclose; `mcp__Claude_in_Chrome__*`, or `mcp__Claude_Preview__*` / `mcp__playwright__*`). The main thread drives it; reviewers stay read-only and assess the captured evidence. Required in `--deep` + UI; an unavailable browser is a disclosed gap, never an error. It drives your **real authenticated** browser and stores screenshots — reads/screenshots may expose cookies/tokens/PII, so prefer a non-production target and never paste sensitive `--report full` screenshots into public PRs/issues.
...
udflow targets **Claude Code**. As of 0.20.0 its **subagents** (the reviewer/`gatekeeper` panel) also load under the **GitHub Copilot CLI**: the agent files now use the `*.agent.md` convention and are explicitly wired via the `plugin.json` `agents` array — the manifest field both harnesses read. Its **skills** already use the `skills/<name>/SKILL.md` layout Copilot also discovers, so they should load there too without changes. This cross-harness loading is derived from each tool's **documented** plugin format and is now **live-verified on GitHub Copilot CLI 1.0.65** (2026-06-26): `copilot plugin list` shows `udflow@kktmarketplace (v0.23.0)`, `copilot skill list` lists both skills, and a live `copilot -p` session enumerated all nine subagents (`implementer` + the seven reviewers + `gatekeeper`); both the SessionStart and Stop hooks were observed firing (hook-debug log). (The README's earlier hook-portability claims are also unit-tested — see the portability suite.)

What stays Claude Code-only is **plan-gate enforcement** and the **deep-mode Workflow**: Copilot has no `plan` permission mode, so the plan gate has nothing to enforce and simply **no-ops** there. The three hooks (`plan-gate`, `load-failure-memory`, `orchestration-check`) resolve the plugin root from the environment at runtime and **fail open** (allow, never error) on any host that runs them differently — so under Copilot they degrade gracefully and never break a session. One verified behavioral difference: Copilot CLI **runs** the hooks but does **not surface their injected output to the model** — the `load-failure-memory` SessionStart `additionalContext` (and the `orchestration-check` Stop advisory) are not delivered, so the **auto failure-memory digest is a no-op under Copilot** (the hook still runs, fail-open). It degrades to the workflow's normal **manual** failure-memory retrieval during planning (the model reads `ai/FAILURE_MEMORY.md` itself), and the readiness discipline is skill/model-driven rather than hook-dependent.
