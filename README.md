...
試試 `/udflow:run <task>`,在 `/agents` 確認各 agent 已載入,編輯後用 `/reload-plugins` 熱重載。

## 驗證 / 發佈

    claude plugin validate .

推上 GitHub 後,其他人即可:先 `/plugin marketplace add simba6507/universal-dev-flow-plugin`,再 `/plugin install udflow@kktmarketplace`。
