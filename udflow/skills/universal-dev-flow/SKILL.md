...
- Do not create or edit a `udflow.*` safety-guard setting (`contractGuard` / `destructiveGuard` / `planGate` / `preserveOnCompact`) to unblock an action a guard just asked or denied about — treat the ask/deny as a stop condition and wait for the user's actual response; a clear, freestanding, unrelated human instruction to configure a project this way is still honored normally.
