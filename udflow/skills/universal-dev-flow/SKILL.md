...
9. Final delivery
   - Follow the final output contract in `references/verification-gate.md`, and for substantial tasks emit the **Run Card** defined there (verdict + checks/reviewers that ran + top findings + auto-fixed + remaining + approximate cost) so the user can see what happened without reading the whole transcript.
