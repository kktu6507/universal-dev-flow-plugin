...
9. Final delivery
   - Follow the final output contract in `references/verification-gate.md`. For substantial tasks emit the **Run Card** (the at-a-glance header) followed by the **Run Report** defined there — a legible, table-based account of **what each agent did, found, and fixed**; an **Outcome** table mapping requirement → change → effect/improvement; a **token & cost breakdown** (per-agent *observed* tokens + an *estimated* orchestrator figure + a grand total + an approximate cost, no telemetry); and **UI/UX evidence** (the after-change screenshot when the task changed UI) — so the user can see what happened, what it achieved, and what it cost without reading the whole transcript.
