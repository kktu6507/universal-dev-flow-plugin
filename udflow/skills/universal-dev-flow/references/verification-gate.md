...
- Contract: when `output/udflow/contract.md` exists, run `scripts/contract-check.mjs` (`references/task-contract.md`) for deterministic scope-diff + AC-coverage evidence; fail-open (absent/unparseable ⇒ no claim).
...
- The **task contract** (`output/udflow/contract.md`, `references/task-contract.md`) is a kept run artifact, covered by the top-level `output/udflow/.gitignore`; never committed into a distributed repo (it is one run's scratch, not the consuming project's source).
