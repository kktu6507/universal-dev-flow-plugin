...
4. **Each hook fires + fails open.** Pipe a synthetic event to each hook with `UDFLOW_HOOK_DEBUG=1`
   ...
   - `printf '{"tool_name":"Write","tool_input":{"file_path":"output/udflow/contract.md","content":"x"}}' | UDFLOW_HOOK_DEBUG=1 node "$R/hooks/contract-guard.js"` — expect exit 0 and a `[contract-guard] …` debug line (no prior file ⇒ first-write case, always allowed).
