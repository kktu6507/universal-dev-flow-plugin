...
4. Update `CHANGELOG.md` for user-visible changes — and bump the version in both manifests (`udflow/.claude-plugin/plugin.json`, `package.json`) when the shipped `udflow/` tree changes. Pure repo-root docs (README, EVIDENCE, this file) don't ship and don't need a bump.
