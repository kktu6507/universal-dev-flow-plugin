...
Note (repo tooling, not shipped): CI now runs `claude plugin validate ./udflow` (the plugin itself) in addition to the marketplace root; `validate-structure.mjs` also asserts the root `package.json` version agrees with the plugin/marketplace/metadata/CHANGELOG versions; and a `release` CI job (runs after Validate passes, push-to-master only, idempotent) auto-publishes the annotated tag + GitHub release from the manifest version — so the version / manifests / CHANGELOG / git tag / release cannot drift and the maintainer never tags by hand.
